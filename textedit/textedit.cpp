...
// #include <c:\Qt\2010.05\qt\src\gui\text\qzip.cpp>
     ...
     : QMainWindow(parent)
 {
     ...
     connect(actionEditForTest,SIGNAL(triggered()),this,SLOT(EditForTest()));
 ...
 void TextEdit::setupFileActions()
 {
     ...
     connect(a, SIGNAL(triggered()), this, SLOT(fileOpen(QString)));
     tb->addAction(a);
     menu->addAction(a);
     ...
     connect(a, SIGNAL(triggered()), this, SLOT(ForSaveWord(QString)));
     tb->addAction(a);
 #endif
 ...
 void TextEdit::setupEditActions()
 {
     ...
     a = actionEditForTest = new QAction(QIcon::fromTheme("edit-paste", QIcon(rsrcPath + "/editpaste.png")),
                                   tr("&test"), this);
     tb->addAction(a);
 #ifndef QT_NO_CLIPBOARD
 ...
 void TextEdit::SaveWord(const QString  &text)
 {
   ...
   str=textEdit->toPlainText();
     ...
     QByteArray dddd= text.toLocal8Bit();
     bytteet = bytteet.replace("kkkkkk",dddd);
     fille.write(bytteet);
     fille.close();
   qDebug()<<bytteet;
     QString program = "7z.exe a -tzip 1.zip ./*";
     qDebug()<<program;
     QProcess *zipProcess  = new QProcess();
     zipProcess->start(program);
     zipProcess->waitForFinished();
 }
bool TextEdit::ForSaveWord( QString &str)
 {
    bool result;
     str=textEdit->toPlainText();
    try
...
//     return true;
// }
 bool TextEdit::fileOpen(const QString &title)
 {
     bool result;
     if (!title.isEmpty())
     {
 ...
 bool TextEdit::fileSaveAs()
 {
     QString fn = QFileDialog::getSaveFileName(this, tr("Save as..."),
                                               QString(), tr("ODF files (*.odt);;xml-Files (*.xml);;All Files (*)"));
     if (fn.isEmpty())
         return false;
     if (! (fn.endsWith(".odt", Qt::CaseInsensitive) || fn.endsWith(".xml", Qt::CaseInsensitive)) )
         fn += ".odt"; //
 ...
 QString  TextEdit::textSize(const QString &p)
 {
 ...
 int TextEdit::textStyle(int styleIndex)
 {
 ...
 void TextEdit::EditForTest()
 {
     textEdit->setText("Pavlova Viktoria is student of BNTU");
     qDebug()<<"test";
